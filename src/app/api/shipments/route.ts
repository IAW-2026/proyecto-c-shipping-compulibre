...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const {
      ...
      courier,
      externalTrackingId,
      // FIX: accept the field names Seller App and Control Plane actually send.
      // Both apps send `sellerId` and `buyerOrderId`, not `externalSellerId`
      // and `externalBuyerOrder`. Accept both spellings so all callers work.
      sellerId,
      externalSellerId,
      buyerOrderId,
      externalBuyerOrder,
    ...
    // Prefer the canonical names Seller/Control Plane send; fall back to the
    // old aliases so nothing breaks if a caller uses the old field names.
    const resolvedSellerId     = sellerId     ?? externalSellerId   ?? "";
    const resolvedBuyerOrderId = buyerOrderId ?? externalBuyerOrder ?? "";
    ...
    const shipment = await prisma.shipment.create({
      data: {
        ...
        externalBuyerOrderId:  buyerOrderId,
        courier,
        originAddress,
